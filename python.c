...
int getnexttoken(FILE *fp, token *tt, int *tp)
{
    ...
    while (x)
    {
        ...
        else if (n == '"')
        {
            ...
            word[k++] = '"';
            word[k] = '\0';
            add_token(tt, tp, word, "literal string", rn, cn - k, 0);
            k = 0;
            return 1;
        }
...
int main()
{
    ...
    for (int i = 0; i < tp; i++)
    {
        ...
        if (strcmp(current_token.token_name, "def") == 0)
        {
